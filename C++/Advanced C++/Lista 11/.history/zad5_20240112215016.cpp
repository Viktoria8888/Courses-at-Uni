...
struct array_in_file {
    ...
    array_in_file(const std::string& fname, const T (&array)[N]) : filename(fname) {
    ...
    T operator[](size_t i) {
        file.seekg(std::ios::beg); // Move to the beginning of the file
        T value;
        for (size_t idx = 0; idx <= i; ++idx) {
            file >> value;
            if (file.peek() == ',') file.ignore(2); // Skip comma and space
        }
        return value;
    }
};
