...
template<typename T, size_t N>
struct arr_holder {
    static const size_t Threshold = 1024 * 1024;
...
int main()
{
    ...
    *smallObj.value = 42;
    ...
    // Maybe add some verification or pause here to ensure the file is written before proceeding
    largeArray.read_from_file(testArray);

    // Verification step
    for (int i = 0; i < 5000; ++i) {
        if (testArray[i] != i) {
            std::cerr << "Mismatch at index " << i << ": expected " << i << ", got " << testArray[i] << std::endl;
            return 1;
        }
    }
    std:
