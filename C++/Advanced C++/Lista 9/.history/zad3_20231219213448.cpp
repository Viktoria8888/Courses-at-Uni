...
void displayFileAttributes(const fs::path &filePath)
{
    ...
    if (fs::exists(filePath))
    {
        ...
        auto modifiedTime = chrono::system_clock::to_time_t(ftime);

        // Output the modified time using std::put_time
        cout << "Last modified time: " << std::put_time(std::localtime(&modifiedTime), "%Y-%m-%d %H:%M:%S") << endl;
